...
% Feedback: bank angle -> rudder
Kphidr = get_feedback_gain(-sysLatAugYawDamper('bank angle', 'rudder'), ...
    0.8, 'Dutch roll', 0, 2);  % positive feedback

% Closed loop system with bank-angle feedback to rudder
sysLatAugClosedLoop2 = feedback(sysLatAugYawDamper, Kphidr, 2, 4, +1);



% % Feedback: sideslip angle -> aileron to stabilize spiral mode
% Kbetada = get_feedback_gain(-sysLatAugYawDamper('sideslip angle', 'aileron'), ...
%     0.7, 'Dutch roll', 0, 2); % positive feedback
% 
% % Closed loop system with sideslip-angle feedback to aileron
% sysLatAugClosedLoop = feedback(sysLatAugYawDamper, Kbetada, 1, 2, +1);
...
Kbetadr = get_feedback_gain(sysLatAugClosedLoop2('sideslip angle', 'rudder'), ...
    0.75, 'Dutch roll', 0.01, 1.0);  % negative feedback

% Closed loop system with all feedback controllers
sysLatAugCL = feedback(sysLatAugClosedLoop2, Kbetadr, 2, 2, -1);


% Feedback: bank angle -> aileron
Kphida = get_feedback_gain(-sysLatAugCL('bank angle', 'aileron'), ...
    0.7, 'Dutch roll', 0.01, 1.0);  % negative feedback

% Closed loop system with all feedback controllers
sysLatAugCL2 = feedback(sysLatAugCL, Kphida, 1, 4, +1);
...
plot_step_response(sysLatAugCL, deg2rad(20), 20);
plot_step_response(uDefl, deg2rad(20), 20);